...
- **LLM** (`TransformersLLMProviderNode`): a local HuggingFace transformers-format chat model directory (`config.json` + `.safetensors` weights + tokenizer files, e.g. `google/gemma-4-12b-it`) with a tokenizer chat template. Place the whole model directory in `models/llm/transformers/<model-dir>/`. FP8-quantized checkpoints (`quant_method: "fp8"`) are supported automatically via the checkpoint's own `config.json` -- no separate setting -- but require a GPU with Compute Capability >= 9 (Hopper H100 or newer, or Blackwell); on hardware without a qualifying GPU/XPU, the model loads anyway as a full-precision (dequantized) model, with a log warning explaining why. Behavior on a CUDA GPU below Compute Capability 9, and whether `triton`/`compressed-tensors` are needed on a qualifying GPU, are both unverified -- confirming either requires real GPU hardware. Other pre-quantized formats (AWQ, GPTQ, NVFP4, etc.) aren't supported yet.
